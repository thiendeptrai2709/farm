...
public class Chest : MonoBehaviour, IInteractable
{
    ...
    public string GetInteractText()
    {
        ...
        // Kiểm tra xem có đang cầm Rìu và rương này do người chơi xây không
        if (isBuiltByPlayer && InventoryManager.Instance != null && InventoryManager.Instance.selectedHotbarIndex != -1)
        {
            ...
            {
                // Gọi hàm kiểm tra xem rương có trống không
                if (!IsEmpty())
                {
                    return "[!] Rương còn đồ, không thể đập";
                }
                return "[E] Phá dỡ rương";
            }
        }

        // Nếu không cầm rìu, hiện chữ Mở Rương bình thường
    ...
    public bool IsEmpty()
    {
        foreach (var slot in chestSlots)
        {
            ...
            {
                return false; // Chỉ cần 1 ô có đồ là tính là không trống!
            }
        }
        return true; // Đã trống hoàn toàn
    }
